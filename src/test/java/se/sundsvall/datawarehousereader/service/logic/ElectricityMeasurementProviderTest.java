...
class ElectricityMeasurementProviderTest {
	...
	@Test
	void testMonthWithEmptyParameters() {
		...
		verify(electricityMonthRepositoryMock).findAllMatching(customerOrgNrCaptor.capture(), facilityIdCaptor.capture(), fromDateCaptor.capture(), toDateCaptor.capture(), pageableCaptor.capture());
	...
	@Test
	void testMonthWithAllParametersSet() {
		final var aggregateOn = MONTH;
		final var searchParams = MeasurementParameters.create();
		...
		when(electricityMonthRepositoryMock.findAllMatching(anyString(), anyString(), any(LocalDateTime.class), any(LocalDateTime.class), any(Pageable.class))).thenReturn(pageMonthMock);
	...
	@Test
	void testDayWithEmptyParameters() {
		final var searchParams = MeasurementParameters.create();
		...
		final var response = provider.getMeasurements(null, DAY, null, null, searchParams);

		verify(electricityDayRepositoryMock).findAllMatching(customerOrgNrCaptor.capture(), facilityIdCaptor.capture(), fromDateCaptor.capture(), toDateCaptor.capture(), pageableCaptor.capture());
	...
	@Test
	void testDayWithAllParametersSet() {
		...
		when(electricityDayRepositoryMock.findAllMatching(anyString(), anyString(), any(LocalDateTime.class), any(LocalDateTime.class), any(Pageable.class))).thenReturn(pageDayMock);
	...
	@Test
	void testMonthForPageLargerThanResultsMaxPage() {

		when(electricityMonthRepositoryMock.findAllMatching(any(), any(), any(), any(), any(Pageable.class))).thenReturn(pageMonthMock);
		...
		verify(electricityMonthRepositoryMock).findAllMatching(any(), any(), any(), any(), pageableCaptor.capture());
	...
	@Test
	void testDayForPageLargerThanResultsMaxPage() {

		when(electricityDayRepositoryMock.findAllMatching(any(), any(), any(), any(), any(Pageable.class))).thenReturn(pageDayMock);
		...
		verify(electricityDayRepositoryMock).findAllMatching(any(), any(), any(), any(), pageableCaptor.capture());
	...
	void testProblemIsThrownWhenNotSupportedAggregation() {
		...
		assertThat(e.getMessage()).isEqualTo("Not Implemented: aggregation 'HOUR' and category 'ELECTRICITY'");
